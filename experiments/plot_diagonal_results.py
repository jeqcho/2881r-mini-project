#!/usr/bin/env python3
"""
Script to plot diagonal sweep results: Zero-shot Accuracy vs ASR metrics.
...
def main():
    print("=" * 60)
    print("Plotting Diagonal Sweep Results")
    print("=" * 60)
    print()

    # Load data
    csv_file = "out/experiments/diagonal_sweep/results_diagonal.csv"
    print(f"Loading data from: {csv_file}")

    if not Path(csv_file).exists():
        print(f"ERROR: Results file not found: {csv_file}")
        print("Please run 'python experiments/collect_diagonal_results.py' first.")
        return
    ...
    required_cols = ['averaged', 'inst_ASR_basic', 'ASR_gcg', 'inst_ASR_multiple_nosys', 'P_Q']
    ...
    # Output directory
    output_dir = Path("out/experiments/diagonal_sweep")
