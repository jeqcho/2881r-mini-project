#!/usr/bin/env python3
"""
Script to plot custom P,Q sweep results: Zero-shot Accuracy vs ASR metrics.
Generates 3 PNG plots for Vanilla ASR, Adv-Suffix ASR, and Adv-Decoding ASR.
Uses same styling as diagonal sweep plots.
"""
...
def main():
    print("=" * 60)
    print("Plotting Custom P,Q Sweep Results")
    print("=" * 60)
    print()

    # Load data
    csv_file = "out/experiments/custom_pq_sweep/results_custom_pq.csv"
    print(f"Loading data from: {csv_file}")

    if not Path(csv_file).exists():
        print(f"ERROR: Results file not found: {csv_file}")
        print("Please run 'python experiments/collect_custom_pq_results.py' first.")
        return
    ...
    # Output directory
    output_dir = Path("out/experiments/custom_pq_sweep")
